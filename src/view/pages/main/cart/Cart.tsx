...
import ShippingOption from '../../../components/molecules/ShippingOption';

const Cart = () => {
	const { cart, subtotal, discountCode, total, shippingOption, shippingCost } = useSelector(
	...
	const handleApplyDiscount = () => {
		const couponInput = document.getElementById('coupon') as HTMLInputElement | null;
						...
						<tbody>
							{cart.map((item: IProduct) => (
								<tr key={item._id} className='border-b border-gray-400'>
									...
									<td className='py-4'>
										<input
											...
											onChange={(e) => {
												...
												if (itemId) {
													handleUpdateQuantity(itemId, parseInt(e.target.value, 10));
												}
											}}
											min={1}
											max={10}
									...
									<td className='py-4'>
										{item && item.price && item.quantity && `$${item.price * item.quantity}`}
									</td>
									<td className='py-4'>
										<button
											className='hover text-red-600'
											onClick={() => item && item._id && handleRemoveProductCart(item._id)}>
