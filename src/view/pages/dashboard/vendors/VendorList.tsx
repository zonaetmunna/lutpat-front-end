...
import DeleteVendor from '../../../components/organisms/DeleteVendor';
import UpdateVendor from '../../../components/organisms/UpdateVendor';
...
const VendorList = () => {
	...
	// update delete
	const [selectedSeller, setSelectedSeller] = useState<ISellerData | null>(null);
	const [isSingleSellerModalOpen, setIsSingleSellerModalOpen] = useState<boolean>(false);
	const [isUpdateSellerModalOpen, setIsUpdateSellerModalOpen] = useState<boolean>(false);
	const [isDeleteSellerModalOpen, setIsDeleteSellerModalOpen] = useState<boolean>(false);

	const [updateSeller] = useUpdateSellerMutation();

	const [deleteSeller] = useDeleteSellerMutation();

	const handleUpdateSeller = ({ sellerId, seller }: { sellerId: string; seller: ISellerData }) => {
				...
				<tbody className='bg-white divide-y divide-gray-200'>
					{visibleSellers?.map((seller: ISellerData) => (
						<tr key={seller._id}>
							...
							<td className='px-6 py-4 whitespace-nowrap'>
								<div className='text-sm text-gray-500'>{seller.profileImage}</div>
							</td>
